...
//	}
	public void secure(CriminaliteService c){	// La capacite des commissariats depend de leur budget ainsi que de l'education / formation re�ue
		...
		if (c.getCrimeMineur() < influence/2){			//50% de l'influence est dirigee vers les crimes mineurs
			influence -= c.getCrimeMineur();
			this.recette += c.getCrimeMineur()*(100-tolerance)*35/100;		// Recette des amendes mineurs
			c.cumulMineur(-c.getCrimeMineur());
		} else {
			influence = influence/2;
			this.recette += influence*(100-tolerance)*35/100;
			c.cumulMineur(influence);
		}
		if (c.getCrimeMoyen() < influence/2){				//25% de l'influence est dirigee vers les crimes moyens
			influence -= c.getCrimeMoyen();				// Si le crime mineur n'est pas assez repandu, les effectifs sont rediriges vers des gestions de crime plus graves
			this.recette += c.getCrimeMoyen()*(100-tolerance)*60/100;			// Recette des amendes intermediaires
			c.cumulMoyen(-c.getCrimeMoyen());
		} else {
			influence = influence/2;
			this.recette += influence*(100-tolerance)*60/100;
			c.cumulMoyen(influence);
		}
		if (c.getCrimeGrave() < influence*4/5){			//20% de l'influence est dirigee vers les crimes graves
			influence -= c.getCrimeGrave();
			this.recette += c.getCrimeGrave()*(100-tolerance)*120/100;			// Recette des amendes lourdes
			c.cumulGrave(-c.getCrimeGrave());
		} else {
